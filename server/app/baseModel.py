...
class BaseModel(Base): # Kế thừa từ Base của ứng dụng
    ...
    id: Mapped[str] = mapped_column(
        String(36),
        primary_key=True,
        default=lambda: str(uuid.uuid4()) # Tạo UUID mới cho mỗi instance
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), # Thêm timezone=True nếu bạn muốn lưu thông tin múi giờ
        default=datetime.utcnow,
        server_default=func.now() # Giá trị mặc định ở phía DB
    )
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
        default=datetime.utcnow,
        onupdate=datetime.utcnow,      # Cập nhật khi đối tượng Python được thay đổi
        server_default=func.now(),     # Giá trị mặc định ở phía DB khi tạo
        server_onupdate=func.now()   # Cập nhật ở phía DB khi có thay đổi
    )
    ...
    def save(self):
        """Lưu instance hiện tại vào database session và commit."""
        try:
            db.session.add(self)
            db.session.commit()
            return self
